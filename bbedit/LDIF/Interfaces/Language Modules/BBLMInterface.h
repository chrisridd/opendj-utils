...
#ifndef PIN
//	We need a separate macro for the token pasting, since it won't happen within the __XX() macros
#define	__VAR__(sym, val) sym##val

#define __PIN(A,B, ctr)	({ __typeof__(A) __VAR__(__a, ctr) = (A); __typeof__(B) __VAR__(__b, ctr) = (B); __VAR__(__a, ctr) > __VAR__(__b, ctr) ? __VAR__(__b, ctr) : __VAR__(__a, ctr); })
#define PIN(A,B)	__PIN(A, B, __COUNTER__)
#endif
...
typedef struct
{
	...
	FourCharCode	fLanguage;					//	what language we're checking
	FourCharCode	fParentLanguage;			//	the parent language whose module we're talking to
...
//	version 1 callbacks
	...
	Boolean		(*fGetRun)(	SInt32 index,					//	get a run
							FourCharCode& language,				//	language code
...
//	version 6 callbacks
	...
	OSErr		(*fFindEmbeddedLanguageRunsInRange)(const FourCharCode language,
													BBLMParamBlock &myParams,
													const SInt32 startOffset,
													const SInt32 rangeLength,
													bool &continueScanning);
	
	OSErr		(*fFindEmbeddedLanguageFunctionsInRange)(const FourCharCode language,
															BBLMParamBlock &myParams,
															const SInt32 startOffset,
															const SInt32 rangeLength);
